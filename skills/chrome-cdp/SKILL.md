...
- Discarded tabs: Chrome unloads background tabs' renderers to save memory. They still show in `list`, but page commands against them fail fast (~3s) with a "renderer not responding" diagnostic instead of hanging. Either focus the tab in Chrome so it reloads, or re-run with `--activate` (or `CDP_ACTIVATE=1`) to wake it. `--activate` foregrounds and reloads the tab, so it is opt-in — it disturbs the user's current foreground tab.
